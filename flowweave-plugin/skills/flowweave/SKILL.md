---
name: flowweave
description: Process the active FlowWeave Agent Inbox request when the user asks to use FlowWeave context or handle the current FlowWeave task.
---
...
5. Write exactly one JSON response to `request.responsePath` through a temporary file followed by an atomic rename.
